...
void run_outstanding_clients(message_data *rcv_buf, message_data *send_buf,
  ...
  struct statistic sample_lat;
  int lat_pick = 0;
  unsigned long total_received = 0, elapsed = 0, partial = 0;
  ...
  do {
    ...
    unsigned long temp = diff_time(current_timep, init_secondp);
    elapsed += temp;
    partial += temp;
    ...
    for (int i = 0; i < nclients; i++)
      if (cl[i].busy &&
          diff_time(current_timep, &cl[i].start_lat) > TIMEOUT_US) {
        cl[i].waiting++;
        cl[i].busy = 0;
    ...
    if (verbose && partial > 999000) {
      printf("Client: %lu value/sec\n", total_received);
      total_received = 0;
      partial = 0;
    }
