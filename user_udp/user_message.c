...
message_data*
create_rcv_message(void)
{
  return create_message(NULL, MAX_MESS_SIZE, -1);
}

message_data*
create_message(char* data, size_t size_data, int id)
{
...
int
get_message_id(message_data* mess)
{
  return mess->id;
}

void
set_message_id(message_data* mess, int id)
{
  mess->id = id;
}

char*
get_message_data(message_data* mess)
{
  return mess->mess_data;
}

size_t
get_message_size(message_data* mess)
{
  return mess->mess_len;
}

size_t
get_total_mess_size(message_data* mess)
{
  return mess->mess_len + sizeof(struct message_data);
}

void
delete_message(message_data* mess)
{
  if (mess)
    free(mess);
}
